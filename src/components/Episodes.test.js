...
test("renders properly with episodes data", () => {
  const { rerender } = render(<Episodes episodes={[]}/>);

  // There should be no episodes on first load
  expect(screen.queryByText(/episode \d/i)).toBeNull();

  // Re-render component with episode data
  rerender(<Episodes episodes={episodeData}/>);

  // There should still be no episodes on the page after data is loaded
  // because we haven't selected a season from the dropdown
  expect(screen.queryAllByText(/episode \d/i)).toHaveLength(8);
});