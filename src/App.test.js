...
test("renders components and updates state appropriately", async () => {
  // Mock response from running fetchShow
  mockFetchShow.mockResolvedValueOnce(mockResponseData);

  render(<App />);

  // There should be no show on load
  expect(screen.queryByText(/stranger things/i)).toBeNull();
  expect(screen.queryByText(/fetching data/i)).toBeInTheDocument();

  // After component mounts, there should be a show
  const showTitle = await waitFor(() => {
    return screen
            .findByRole('heading',
  ...
  // Test the dropdown by selecting a season
  userEvent.click(dropdown);

  const dropdownOptions = screen
    .getAllByRole("option", { name: /season \d/i });

  // Select season 2
  userEvent.click(dropdownOptions[1]);

  // Season 2 episodes should be displayed --> Episode 1: MADMAX
  expect(screen.getByText(/madmax/i)).toBeInTheDocument();
